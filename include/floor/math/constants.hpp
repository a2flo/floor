...
namespace fl::const_math {
	...
	using max_fp_type = long double;
#else
	using max_fp_type = double; // can only use double on compute/graphics backends
#endif
	
	//! largest supported floating point type at run-time
#if !defined(FLOOR_DEVICE) || (defined(FLOOR_DEVICE_HOST_COMPUTE) && !defined(FLOOR_DEVICE_HOST_COMPUTE_IS_DEVICE))
	using max_rt_fp_type = long double;
#elif !defined(FLOOR_DEVICE_NO_DOUBLE)
	using max_rt_fp_type = double; // can only use double on compute/graphics backends
#else
	using max_rt_fp_type = float; // or even only float when there is no double support
#endif
