...
public:
	~device_buffer() override = default;
	...
	virtual void write(const device_queue& cqueue, const void* src, const size_t size = 0, const size_t offset = 0) = 0;
	...
	template <typename T> floor_inline_always void write_from(const T& src, const device_queue& cqueue, const size_t offset_ = 0) {
	...
	virtual void copy(const device_queue& cqueue, const device_buffer& src,
	...
	virtual bool fill(const device_queue& cqueue,
	...
	//! NOTE: "cqueue" must be a device_queue of the compute context, "vk_queue" must be a device_queue of the Vulkan context
	virtual bool acquire_vulkan_buffer(const device_queue* cqueue floor_unused, const vulkan_queue* vk_queue floor_unused) const {
		return false;
	}
	//! releases the associated Vulkan buffer from use with compute (-> acquire for Vulkan use)
	//! NOTE: "cqueue" must be a device_queue of the compute context, "vk_queue" must be a device_queue of the Vulkan context
	virtual bool release_vulkan_buffer(const device_queue* cqueue floor_unused, const vulkan_queue* vk_queue floor_unused) const {
		return false;
	}
	...
	virtual bool sync_vulkan_buffer(const device_queue* cqueue floor_unused, const vulkan_queue* vk_queue floor_unused) const {
		return false;
	}
	...
	bool dump_to_file(const device_queue& cqueue, const std::string& file_name) {
		return device_memory::dump_to_file<value_type>(*this, size, cqueue, file_name);
	}
	...
	bool dump_binary_to_file(const device_queue& cqueue, const std::string& file_name) {
		return device_memory::dump_binary_to_file(*this, size, cqueue, file_name);
	}
...
protected:
	...
	union {
		device_buffer* shared_buffer { nullptr };
	...
	// NOTE: this can also be enabled by simply defining FLOOR_DEBUG_DEVICE_BUFFER elsewhere
#if defined(FLOOR_DEBUG) || defined(FLOOR_DEBUG_DEVICE_BUFFER)
	floor_inline_always static bool read_check(const size_t& buffer_size,
											   const size_t& read_size,
											   const size_t& offset,
											   const MEMORY_FLAG& buffer_flags) {
		if(read_size == 0) {
		...
		if(!has_flag<MEMORY_FLAG::HOST_READ>(buffer_flags)) {
	...
	floor_inline_always static bool write_check(const size_t& buffer_size,
		...
		if(!has_flag<MEMORY_FLAG::HOST_WRITE>(buffer_flags)) {
	...
	floor_inline_always static bool map_check(const size_t& buffer_size,
											  ...
											  const MEMORY_FLAG& buffer_flags,
											  const MEMORY_MAP_FLAG& map_flags,
											  const size_t& offset) {
		if(has_flag<MEMORY_MAP_FLAG::WRITE_INVALIDATE>(map_flags) &&
		   (map_flags & MEMORY_MAP_FLAG::READ_WRITE) != MEMORY_MAP_FLAG::NONE) {
		...
		if((buffer_flags & MEMORY_FLAG::HOST_READ_WRITE) == MEMORY_FLAG::NONE) {
		...
		if((buffer_flags & MEMORY_FLAG::HOST_READ_WRITE) != MEMORY_FLAG::HOST_READ_WRITE) {
			if(has_flag<MEMORY_FLAG::HOST_READ>(buffer_flags) &&
			   (has_flag<MEMORY_MAP_FLAG::WRITE>(map_flags) || has_flag<MEMORY_MAP_FLAG::WRITE_INVALIDATE>(map_flags))) {
				log_error("map: buffer has been created with the HOST_READ flag, but map flags specify buffer must be writable!");
				return false;
			}
			if(has_flag<MEMORY_FLAG::HOST_WRITE>(buffer_flags) &&
			   has_flag<MEMORY_MAP_FLAG::READ>(map_flags)) {
	...
	floor_inline_always static bool map_check(const size_t&, const size_t&,
											  const MEMORY_FLAG&, const MEMORY_MAP_FLAG&,
											  const size_t&) {
		return true;
	}
#endif
