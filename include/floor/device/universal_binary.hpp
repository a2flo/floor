...
//! binary format:
//! [magic: char[4] = "FUBA"]
//! [binary format version: uint32_t = 7]
//! [binary count: uint32_t]
//! [FUBAR flags: uint32_t]
//! [binary targets: target_v7[binary count]]
...
//!         [function info version: uint32_t = 7]
...
using namespace std::literals;
	...
	static constexpr const uint32_t binary_format_version { 7u };
	//! current version of the target format
	static constexpr const uint32_t target_format_version { 7u };
	//! current version of the function info
	static constexpr const uint32_t function_info_version { 7u };
	...
	union __attribute__((packed)) target_v7 {
...
#define FLOOR_FUBAR_VERSION_AND_TYPE \
		...
		struct __attribute__((packed)) {
			...
			//! if set, enables subgroup uniform control flow support
			uint64_t subgroup_uniform_cf_support : 1;
			
			//! if set, only assumes 4 rather than 16 IUBs are available
			uint64_t low_iub_count : 1;
		...
		bool operator==(const target_v7& cmp) const {
			return (value == cmp.value);
		}
		
#undef FLOOR_FUBAR_VERSION_AND_TYPE // cleanup
	};
	static_assert(sizeof(target_v7) == sizeof(uint64_t));
	
	//! static part of the universal binary archive header (these are the first bytes)
	struct __attribute__((packed)) header_v7 {
		//! magic identifier
	...
	static_assert(sizeof(header_v7) == sizeof(uint32_t) * 4u);
	
	//! extended/dynamic part of the header
	struct header_dynamic_v7 {
		//! static part of the header
		header_v7 static_header;
		//! binary targets
	...
	struct function_info_dynamic_v7 {
		//! static part of the function info
		function_info_v7 static_function_info;
		//! function name
		std::string name;
	...
	struct __attribute__((packed)) binary_v7 {
	...
	static_assert(sizeof(binary_v7) == sizeof(uint32_t) * 4);
	
	//! per-binary header (dynamic part)
	struct binary_dynamic_v7 {
		//! static part of the binary header
		binary_v7 static_binary_header;
		//! function info for all contained functions
		std::vector<function_info_dynamic_v7> function_info;
		//! binary data
	...
	struct archive {
		...
		std::vector<binary_dynamic_v7> binaries;
	};
	
	//! aliases for current formats
	using target = target_v7;
	using header = header_v7;
	using header_dynamic = header_dynamic_v7;
	using function_info = function_info_v7;
	using function_info_dynamic = function_info_dynamic_v7;
	using binary = binary_v7;
	using binary_dynamic = binary_dynamic_v7;
	...
	struct archive_binaries {
		...
		//! matching binaries
		std::vector<std::pair<const binary_dynamic_v7*, const target_v7>> dev_binaries;
	};
