...
class metal_device;
class device;
class metal4_buffer final : public device_buffer {
public:
	metal4_buffer(const device_queue& cqueue,
				  const size_t& size_,
	...
	metal4_buffer(const device_queue& cqueue,
				  ...
				  const MEMORY_FLAG flags_ = (MEMORY_FLAG::READ_WRITE |
											  MEMORY_FLAG::HOST_READ_WRITE),
				  const char* floor_nullable debug_label_ = nullptr) :
	metal4_buffer(cqueue, size_, {}, flags_, debug_label_) {}
	...
	//! helper function to synchronize Metal resource data between the CPU and GPU
