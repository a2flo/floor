...
namespace metal_args {
	...
	static inline void arg_post_handler(const function_info& entry, idx_handler& idx, const device_function_arg& arg) {
		// advance all indices
		if (idx.is_implicit) {
			++idx.implicit;
			...
			if (entry.args[idx.arg].image_type == ARG_IMAGE_TYPE::NONE) {
				// buffer
				if (auto vec_buf_ptrs = get_if<const std::vector<device_buffer*>*>(&arg.var)) {
				...
				if (auto vec_img_ptrs = get_if<const std::vector<device_image*>*>(&arg.var)) {
