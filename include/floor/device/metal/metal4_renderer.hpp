...
class metal4_renderer final : public graphics_renderer {
public:
	metal4_renderer(const device_queue& cqueue_,
					const graphics_pass& pass_,
					const graphics_pipeline& pipeline_,
					const bool multi_view_ = false);
	~metal4_renderer() override;
...
protected:
	friend metal4_renderer_internal;
	
	metal4_command_buffer cmd_buffer;
	id <MTL4RenderCommandEncoder> encoder;
	MTL4RenderPassDescriptor* mtl_pass_desc { nullptr };
	...
	const metal4_pipeline::metal4_pipeline_entry* mtl_pipeline_state { nullptr };
