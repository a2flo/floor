...
#define FLOOR_DEVICE_HAS_SOFT_PRINTF 1
...
floor_inline_always const_func global uint32_t* floor_get_printf_buffer() {
	return floor_host_compute_device_printf_buffer();
}
...
#if defined(FLOOR_DEBUG) && !defined(FLOOR_DEVICE_HOST_COMPUTE_IS_DEVICE)
...
#pragma clang attribute push (__attribute__((weakref("floor_host_compute_device_barrier"))), apply_to=function)
...
#if !defined(FLOOR_DEVICE_HOST_COMPUTE_IS_DEVICE) // host-only (host-device deals with local memory differently)
// local memory management (NOTE: implemented in host_function.cpp)
