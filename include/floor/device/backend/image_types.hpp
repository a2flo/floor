...
//! image type
enum class IMAGE_TYPE : uint64_t {
	...
	RG11B10F				= CHANNELS_3 | FORMAT_11_11_10 | FLOAT,
	RGB9E5F					= CHANNELS_3 | FORMAT_9_9_9_EXP_5 | FLOAT,
	...
	BC6H_RGBHF				= BPTC_RGBHF,
	BC6H_RGBUHF				= BPTC_RGBUHF,
...
floor_global_enum_ext(IMAGE_TYPE)

//! returns true if the image layout is R, RG, RGB or RGBA
floor_inline_always static constexpr bool image_layout_rgba(const IMAGE_TYPE& image_type) {
	return ((image_type & IMAGE_TYPE::__LAYOUT_MASK) == IMAGE_TYPE::LAYOUT_RGBA);
}

//! returns true if the image layout is ABGR or BGR
floor_inline_always static constexpr bool image_layout_abgr(const IMAGE_TYPE& image_type) {
	return ((image_type & IMAGE_TYPE::__LAYOUT_MASK) == IMAGE_TYPE::LAYOUT_ABGR);
}

//! returns true if the image layout is BGRA
floor_inline_always static constexpr bool image_layout_bgra(const IMAGE_TYPE& image_type) {
	return ((image_type & IMAGE_TYPE::__LAYOUT_MASK) == IMAGE_TYPE::LAYOUT_BGRA);
}

//! returns true if the image layout is ARGB
floor_inline_always static constexpr bool image_layout_argb(const IMAGE_TYPE& image_type) {
	return ((image_type & IMAGE_TYPE::__LAYOUT_MASK) == IMAGE_TYPE::LAYOUT_ARGB);
}

//! returns the dimensionality of the specified image type
floor_inline_always static constexpr uint32_t image_dim_count(const IMAGE_TYPE& image_type) {
	return uint32_t(image_type & IMAGE_TYPE::__DIM_MASK) >> uint32_t(IMAGE_TYPE::__DIM_SHIFT);
}

//! returns the storage dimensionality of the specified image type
floor_inline_always static constexpr uint32_t image_storage_dim_count(const IMAGE_TYPE& image_type) {
	return image_dim_count(image_type) + (has_flag<IMAGE_TYPE::FLAG_CUBE>(image_type) ||
										  has_flag<IMAGE_TYPE::FLAG_ARRAY>(image_type) ? 1 : 0);
}

//! returns the channel count of the specified image type
floor_inline_always static constexpr uint32_t image_channel_count(const IMAGE_TYPE& image_type) {
	return (uint32_t(image_type & IMAGE_TYPE::__CHANNELS_MASK) >> uint32_t(IMAGE_TYPE::__CHANNELS_SHIFT)) + 1u;
}

//! return the format of the specified image type
floor_inline_always static constexpr uint32_t image_format(const IMAGE_TYPE& image_type) {
	return uint32_t(image_type & IMAGE_TYPE::__FORMAT_MASK);
}

//! return the anisotropy of the specified image type
floor_inline_always static constexpr uint32_t image_anisotropy(const IMAGE_TYPE& image_type) {
	return uint32_t(1ull << (uint64_t(image_type & IMAGE_TYPE::__ANISOTROPY_MASK) >>
							 uint64_t(IMAGE_TYPE::__ANISOTROPY_SHIFT)));
}

//! return the sample count of the specified image type
floor_inline_always static constexpr uint32_t image_sample_count(const IMAGE_TYPE& image_type) {
	if (!has_flag<IMAGE_TYPE::FLAG_MSAA>(image_type)) return 1u;
	return uint32_t(1ull << (uint64_t(image_type & IMAGE_TYPE::__SAMPLE_COUNT_MASK) >>
							 uint64_t(IMAGE_TYPE::__SAMPLE_COUNT_SHIFT)));
}

//! return the IMAGE_TYPE::SAMPLE_COUNT_* type matching the specified sample count
//! NOTE: "sample_count" must be in [0, 64]
floor_inline_always static constexpr IMAGE_TYPE image_sample_type_from_count(const uint32_t& sample_count) {
	static_assert(uint32_t(IMAGE_TYPE::SAMPLE_COUNT_1) == uint32_t(IMAGE_TYPE::NONE), "sample count definition changed");
	if (sample_count <= 1 || sample_count > 64u) {
		return IMAGE_TYPE::NONE;
	}
	if (sample_count <= 2u) {
		return IMAGE_TYPE::SAMPLE_COUNT_2;
	} else if (sample_count <= 4u) {
		return IMAGE_TYPE::SAMPLE_COUNT_4;
	} else if (sample_count <= 8u) {
		return IMAGE_TYPE::SAMPLE_COUNT_8;
	} else if (sample_count <= 16u) {
		return IMAGE_TYPE::SAMPLE_COUNT_16;
	} else if (sample_count <= 32u) {
		return IMAGE_TYPE::SAMPLE_COUNT_32;
	}
	return IMAGE_TYPE::SAMPLE_COUNT_64;
}
...
floor_inline_always static constexpr uint32_t image_coordinate_width(const IMAGE_TYPE& image_type) {
...
floor_inline_always static constexpr bool image_compressed(const IMAGE_TYPE& image_type) {
	return ((image_type & IMAGE_TYPE::__COMPRESSION_MASK) != IMAGE_TYPE::UNCOMPRESSED);
}
...
floor_inline_always static constexpr bool image_format_valid(const IMAGE_TYPE& image_type) {
	...
	switch(image_type & IMAGE_TYPE::__FORMAT_MASK) {
		case IMAGE_TYPE::FORMAT_3_3_2: return (channel_count == 3);
		case IMAGE_TYPE::FORMAT_5_5_5: return (channel_count == 3);
		case IMAGE_TYPE::FORMAT_5_5_5_ALPHA_1: return (channel_count == 4);
		case IMAGE_TYPE::FORMAT_5_6_5: return (channel_count == 3);
		case IMAGE_TYPE::FORMAT_9_9_9_EXP_5: return (channel_count == 3);
		case IMAGE_TYPE::FORMAT_10: return (channel_count == 3 || channel_count == 4);
		case IMAGE_TYPE::FORMAT_10_10_10_ALPHA_2: return (channel_count == 4);
		case IMAGE_TYPE::FORMAT_11_11_10: return (channel_count == 3);
		case IMAGE_TYPE::FORMAT_12_12_12: return (channel_count == 3);
		case IMAGE_TYPE::FORMAT_12_12_12_12: return (channel_count == 4);
		case IMAGE_TYPE::FORMAT_24: return (channel_count == 1);
		case IMAGE_TYPE::FORMAT_24_8: return (channel_count == 2);
		case IMAGE_TYPE::FORMAT_32_8: return (channel_count == 2);
		case IMAGE_TYPE::FORMAT_8_8_8_ALPHA_1:
		case IMAGE_TYPE::FORMAT_11:
...
static constexpr uint32_t image_bits_per_pixel(const IMAGE_TYPE& image_type) {
	const auto format = image_type & IMAGE_TYPE::__FORMAT_MASK;
	const auto channel_count = image_channel_count(image_type);
	if (!image_compressed(image_type)) {
		const auto sample_count = image_sample_count(image_type);
		switch (format) {
			// arbitrary channel formats
			case IMAGE_TYPE::FORMAT_2: return 2 * channel_count * sample_count;
			case IMAGE_TYPE::FORMAT_4: return 4 * channel_count * sample_count;
			case IMAGE_TYPE::FORMAT_8: return 8 * channel_count * sample_count;
			case IMAGE_TYPE::FORMAT_10: return 10 * channel_count * sample_count;
			case IMAGE_TYPE::FORMAT_16: return 16 * channel_count * sample_count;
			case IMAGE_TYPE::FORMAT_32: return 32 * channel_count * sample_count;
			case IMAGE_TYPE::FORMAT_64: return 64 * channel_count * sample_count;
				
			// special channel specific formats
			case IMAGE_TYPE::FORMAT_3_3_2: return 8 * sample_count;
			case IMAGE_TYPE::FORMAT_5_5_5: return 15 * sample_count;
			case IMAGE_TYPE::FORMAT_5_5_5_ALPHA_1: return 16 * sample_count;
			case IMAGE_TYPE::FORMAT_5_6_5: return 16 * sample_count;
			case IMAGE_TYPE::FORMAT_9_9_9_EXP_5: return 32 * sample_count;
			case IMAGE_TYPE::FORMAT_10_10_10_ALPHA_2: return 32 * sample_count;
			case IMAGE_TYPE::FORMAT_11_11_10: return 32 * sample_count;
			case IMAGE_TYPE::FORMAT_12_12_12: return 36 * sample_count;
			case IMAGE_TYPE::FORMAT_12_12_12_12: return 48 * sample_count;
			case IMAGE_TYPE::FORMAT_24: return 24 * sample_count;
			case IMAGE_TYPE::FORMAT_24_8: return 32 * sample_count;
			case IMAGE_TYPE::FORMAT_32_8: return 40 * sample_count;
			default: return 1 * sample_count;
		}
	} else {
		switch (image_type & IMAGE_TYPE::__COMPRESSION_MASK) {
			case IMAGE_TYPE::PVRTC:
				return (format == IMAGE_TYPE::FORMAT_2 ? 2u : 4u);
			case IMAGE_TYPE::BC1:
				return 4u;
			case IMAGE_TYPE::RGTC:
				return (image_channel_count(image_type) == 1u ? 4u : 8u);
			case IMAGE_TYPE::BC2:
			case IMAGE_TYPE::BC3:
			case IMAGE_TYPE::BPTC:
				return 8u;
			case IMAGE_TYPE::EAC:
			case IMAGE_TYPE::ETC2:
				return (channel_count == 1u || channel_count == 3u ? 4u : 8u);
			case IMAGE_TYPE::ASTC:
...
static constexpr uint32_t image_bits_of_channel(const IMAGE_TYPE& image_type, const uint32_t& channel) {
	...
	switch (image_type & IMAGE_TYPE::__FORMAT_MASK) {
		// arbitrary channel formats
		case IMAGE_TYPE::FORMAT_2: return 2 * sample_count;
		case IMAGE_TYPE::FORMAT_4: return 4 * sample_count;
		case IMAGE_TYPE::FORMAT_8: return 8 * sample_count;
		case IMAGE_TYPE::FORMAT_16: return 16 * sample_count;
		case IMAGE_TYPE::FORMAT_32: return 32 * sample_count;
		case IMAGE_TYPE::FORMAT_64: return 64 * sample_count;
			
		// special channel specific formats
		case IMAGE_TYPE::FORMAT_3_3_2: return (channel <= 1 ? 3 : 2) * sample_count;
		case IMAGE_TYPE::FORMAT_5_5_5: return 5 * sample_count;
		case IMAGE_TYPE::FORMAT_5_5_5_ALPHA_1: return (channel <= 2 ? 5 : 1) * sample_count;
		case IMAGE_TYPE::FORMAT_5_6_5: return (channel == 1 ? 6 : 5) * sample_count;
		case IMAGE_TYPE::FORMAT_9_9_9_EXP_5: return (channel <= 2 ? 14 : 0) * sample_count; // tricky
		case IMAGE_TYPE::FORMAT_10: return 10 * sample_count;
		case IMAGE_TYPE::FORMAT_10_10_10_ALPHA_2: return (channel <= 2 ? 10 : 2) * sample_count;
		case IMAGE_TYPE::FORMAT_11_11_10: return (channel <= 1 ? 11 : 10) * sample_count;
		case IMAGE_TYPE::FORMAT_12_12_12: return 12 * sample_count;
		case IMAGE_TYPE::FORMAT_12_12_12_12: return 12 * sample_count;
		case IMAGE_TYPE::FORMAT_24: return 24 * sample_count;
		case IMAGE_TYPE::FORMAT_24_8: return (channel == 0 ? 24 : 8) * sample_count;
		case IMAGE_TYPE::FORMAT_32_8: return (channel == 0 ? 32 : 8) * sample_count;
		default: return 0;
	}
}
...
static constexpr uint32_t image_bytes_per_pixel(const IMAGE_TYPE& image_type) {
	const auto bpp = image_bits_per_pixel(image_type);
	return ((bpp + 7u) / 8u);
}

//! returns the 2D block size of the compression method that is being used
static constexpr uint2 image_compression_block_size(const IMAGE_TYPE& image_type) {
	if (!image_compressed(image_type)) {
		return {};
	}
	switch (image_type & IMAGE_TYPE::__COMPRESSION_MASK) {
		case IMAGE_TYPE::PVRTC:
			return {
				(image_type & IMAGE_TYPE::__FORMAT_MASK) == IMAGE_TYPE::FORMAT_2 ? 8u : 4u,
				4u,
			};
		case IMAGE_TYPE::BC1:
		case IMAGE_TYPE::BC2:
...
static constexpr size_t image_slice_data_size_from_types(const uint4& image_dim,
	...
	size_t size = size_t(std::max(image_dim.x, min_block_size.x));
	if (dim_count >= 2) size *= size_t(std::max(image_dim.y, min_block_size.y));
	if (dim_count == 3) size *= size_t(image_dim.z);
	
	if (has_flag<IMAGE_TYPE::FLAG_MSAA>(image_type)) {
...
static constexpr uint32_t image_mip_level_count(const uint4& image_dim, const IMAGE_TYPE image_type) {
...
static constexpr uint32_t image_layer_count(const uint4& image_dim, const IMAGE_TYPE image_type) {
	const auto dim_count = image_dim_count(image_type);
...
static constexpr size_t image_data_size_from_types(const uint4& image_dim,
	...
	const auto dim_count = image_dim_count(image_type);
	const auto mip_levels = (ignore_mip_levels ? 1 : std::min(image_mip_level_count(image_dim, image_type), max_mip_level));
...
static constexpr size_t image_mip_level_data_size_from_types(const uint4& image_dim,
	...
	const uint4 mip_image_dim {
		std::max(image_dim.x >> mip_level, min_block_size.x),
		dim_count >= 2 ? std::max(image_dim.y >> mip_level, min_block_size.y) : 0u,
...
template <IMAGE_TYPE image_type, size_t size>
requires((image_type & IMAGE_TYPE::__DATA_TYPE_MASK) == IMAGE_TYPE::UINT && size <= 64u)
struct image_sized_data_type<image_type, size> {
	using type = std::conditional_t<(size == 0u), uint32_t, std::conditional_t<
							   (size > 0u && size <= 8u), uint8_t, std::conditional_t<
							   (size > 8u && size <= 16u), uint16_t, std::conditional_t<
							   (size > 16u && size <= 32u), uint32_t, std::conditional_t<
							   (size > 32u && size <= 64u), uint64_t, void>>>>>;
};
template <IMAGE_TYPE image_type, size_t size>
requires((image_type & IMAGE_TYPE::__DATA_TYPE_MASK) == IMAGE_TYPE::INT && size <= 64u)
struct image_sized_data_type<image_type, size> {
	using type = std::conditional_t<(size == 0u), int32_t, std::conditional_t<
							   (size > 0u && size <= 8u), int8_t, std::conditional_t<
							   (size > 8u && size <= 16u), int16_t, std::conditional_t<
							   (size > 16u && size <= 32u), int32_t, std::conditional_t<
							   (size > 32u && size <= 64u), int64_t, void>>>>>;
};
template <IMAGE_TYPE image_type, size_t size>
requires((image_type & IMAGE_TYPE::__DATA_TYPE_MASK) == IMAGE_TYPE::FLOAT && size <= 64u)
struct image_sized_data_type<image_type, size> {
	using type = std::conditional_t<(size == 0u), float, std::conditional_t<
							   (size > 0u && size <= 16u), float /* no half type, load/stores via float */, std::conditional_t<
							   (size > 16u && size <= 32u), float, std::conditional_t<
							   (size > 32u && size <= 64u), double, void>>>>;
};

//! data type of a single image channel (always 32-bit), used for image reads and writes
template <IMAGE_TYPE image_type>
requires((image_type & IMAGE_TYPE::__DATA_TYPE_MASK) != IMAGE_TYPE::NONE)
struct image_tex_channel_data_type {
	using type = typename image_sized_data_type<image_type, 32u>::type;
};

//! image texel data type used for image reads and writes: vector { 1, 2, 3, 4 } < { float, uint32_t, int32_t } >
template <IMAGE_TYPE image_type>
struct image_texel_data_type {
	using type = vector_n<typename image_tex_channel_data_type<image_type>::type, image_channel_count(image_type)>;
};

//! fits a 4-component vector to the corresponding image data vector type, or passthrough for scalar values
template <IMAGE_TYPE image_type, typename data_type> struct image_vec_ret_type {};
template <IMAGE_TYPE image_type, typename data_type>
...
static constexpr const IMAGE_TYPE image_type_checking_mask {
	IMAGE_TYPE::__DIM_MASK |
	IMAGE_TYPE::FLAG_ARRAY | IMAGE_TYPE::FLAG_BUFFER | IMAGE_TYPE::FLAG_MSAA |
	IMAGE_TYPE::FLAG_CUBE | IMAGE_TYPE::FLAG_DEPTH | IMAGE_TYPE::FLAG_STENCIL
};

//! returns true if the specified image type is a 1D image
static constexpr bool is_image_1d(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_1D);
}

//! returns true if the specified image type is a 1D image array
static constexpr bool is_image_1d_array(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_1D_ARRAY);
}

//! returns true if the specified image type is a 1D image buffer
static constexpr bool is_image_1d_buffer(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_1D_BUFFER);
}

//! returns true if the specified image type is a 2D image
static constexpr bool is_image_2d(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_2D);
}

//! returns true if the specified image type is a 2D image array
static constexpr bool is_image_2d_array(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_2D_ARRAY);
}

//! returns true if the specified image type is a 2D MSAA image
static constexpr bool is_image_2d_msaa(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_2D_MSAA);
}

//! returns true if the specified image type is a 2D MSAA image array
static constexpr bool is_image_2d_msaa_array(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_2D_MSAA_ARRAY);
}

//! returns true if the specified image type is a cube image
static constexpr bool is_image_cube(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_CUBE);
}

//! returns true if the specified image type is a cube image array
static constexpr bool is_image_cube_array(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_CUBE_ARRAY);
}

//! returns true if the specified image type is a 2D depth image
static constexpr bool is_image_depth(IMAGE_TYPE image_type) {
	return ((image_type & (image_type_checking_mask | IMAGE_TYPE::__CHANNELS_MASK)) == IMAGE_TYPE::IMAGE_DEPTH);
}

//! returns true if the specified image type is a 2D depth/stencil image
static constexpr bool is_image_depth_stencil(IMAGE_TYPE image_type) {
	return ((image_type & (image_type_checking_mask | IMAGE_TYPE::__CHANNELS_MASK)) == IMAGE_TYPE::IMAGE_DEPTH_STENCIL);
}

//! returns true if the specified image type is a 2D depth image array
static constexpr bool is_image_depth_array(IMAGE_TYPE image_type) {
	return ((image_type & (image_type_checking_mask | IMAGE_TYPE::__CHANNELS_MASK)) == IMAGE_TYPE::IMAGE_DEPTH_ARRAY);
}

//! returns true if the specified image type is a cube depth image
static constexpr bool is_image_depth_cube(IMAGE_TYPE image_type) {
	return ((image_type & (image_type_checking_mask | IMAGE_TYPE::__CHANNELS_MASK)) == IMAGE_TYPE::IMAGE_DEPTH_CUBE);
}

//! returns true if the specified image type is a cube depth image array
static constexpr bool is_image_depth_cube_array(IMAGE_TYPE image_type) {
	return ((image_type & (image_type_checking_mask | IMAGE_TYPE::__CHANNELS_MASK)) == IMAGE_TYPE::IMAGE_DEPTH_CUBE_ARRAY);
}

//! returns true if the specified image type is a 2D MSAA depth image
static constexpr bool is_image_depth_msaa(IMAGE_TYPE image_type) {
	return ((image_type & (image_type_checking_mask | IMAGE_TYPE::__CHANNELS_MASK)) == IMAGE_TYPE::IMAGE_DEPTH_MSAA);
}

//! returns true if the specified image type is a 2D MSAA depth image array
static constexpr bool is_image_depth_msaa_array(IMAGE_TYPE image_type) {
	return ((image_type & (image_type_checking_mask | IMAGE_TYPE::__CHANNELS_MASK)) == IMAGE_TYPE::IMAGE_DEPTH_MSAA_ARRAY);
}

//! returns true if the specified image type is a 3D image
static constexpr bool is_image_3d(IMAGE_TYPE image_type) {
	return ((image_type & image_type_checking_mask) == IMAGE_TYPE::IMAGE_3D);
}
