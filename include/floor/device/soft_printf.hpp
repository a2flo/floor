...
static inline void handle_printf_buffer(const std::span<uint32_t> buf) {
	...
	const uint32_t bytes_written = std::min(buf[0], printf_buffer_size);
		...
		for (auto ch = format_str.cbegin(); ch != format_str.cend(); ) {
			...
			do {
				...
				switch (*ch) {
					...
					case 'x': // hex
						sstr << std::hex;
						if (cur_ch == 'X') sstr << std::uppercase;
						sstr << cur_arg->u32;
						if (cur_ch == 'X') sstr << std::nouppercase;
						sstr << std::dec;
						break;
					case 'o': // octal
						sstr << std::oct;
...
static inline void initialize_printf_buffer(const device_queue& dev_queue, device_buffer& printf_buffer) {
