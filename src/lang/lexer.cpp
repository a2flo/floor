...
void lexer::map_characters(translation_unit& tu) {
	...
	std::vector<uint32_t> lines; // NOTE: we can't store iterators yet, so this must be a uint
...
lexer::lex_return_type lexer::handle_error(const translation_unit& tu, const source_iterator& iter, const std::string& error_msg) {
	...
	if(line_and_column.first != 0) {
		...
		const std::string line_str = tu.source.substr(size_t(std::distance(*lines_begin, *line_start)), size_t(*line_end - *line_start) - 1u);
		log_undecorated("$", line_str);
		
		// print '^' at erroneous character if possible
		if(line_and_column.second != 0) {
			const std::string caret_string(line_and_column.second - 1, ' ');
...
void lexer::print_tokens(const translation_unit& tu) {
	...
	for(const auto& token : tu.tokens) {
		...
		const auto column_num = (uint32_t)std::distance(*cur_line, tok_begin);
		
		tmp.append(std::to_string(line_num));
		tmp.append(":");
		tmp.append(std::to_string(column_num));
		tmp.append(": ");
...
lexer::lex_return_type lexer::lex_character_constant(const translation_unit& tu, source_iterator& iter, const source_iterator& source_end) {
	...
	if(*iter == '\\') {
		...
		if(!is_escape_sequence_char(iter)) {
			const std::string invalid_seq = (is_printable_char(iter) ? std::string(1, *iter) : "<"+std::to_string(0xFFu & (uint32_t)*iter)+">");
...
lexer::lex_return_type lexer::lex_punctuator(const translation_unit&, source_iterator& iter, const source_iterator& source_end) {
	...
	const auto check_double_char_punctuator = [&iter, &source_end](std::initializer_list<const char> chars) -> source_iterator {
		auto next_iter = (iter + 1);
...
std::pair<uint32_t, uint32_t> lexer::get_line_and_column_from_iter(const translation_unit& tu,
	...
	ret.first = (uint32_t)std::distance(lines_begin, line_iter);
	
	// column num
	const auto prev_newline = *prev(line_iter);
	ret.second = (uint32_t)std::distance(prev_newline, iter);
