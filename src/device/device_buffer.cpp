...
std::shared_ptr<device_buffer> device_buffer::clone(const device_queue& cqueue, const bool copy_contents,
	...
	std::shared_ptr<device_buffer> ret;
...
const metal_buffer* device_buffer::get_underlying_metal_buffer_safe() const {
...
const vulkan_buffer* device_buffer::get_underlying_vulkan_buffer_safe() const {
	if (has_flag<MEMORY_FLAG::VULKAN_SHARING>(flags)) {
		...
		if (auto test_cast_vk_buffer = dynamic_cast<const vulkan_buffer*>(ret); !test_cast_vk_buffer) {
			throw std::runtime_error("specified buffer is neither a Vulkan buffer nor a shared Vulkan buffer");
		}
#endif
		return ret;
	}
