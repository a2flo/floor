...
cuda_buffer::cuda_buffer(const device_queue& cqueue,
						 const size_t& size_,
...
device_buffer(cqueue, size_, host_data_, flags_, shared_buffer_) {
	...
	if (cuda_dev.ctx != nullptr) {
		if (!cuda_dev.make_context_current()) {
			throw std::runtime_error("failed to make CUDA context current for buffer allocation");
		}
	}
...
void cuda_buffer::write(const device_queue& cqueue, const void* src, const size_t size_, const size_t offset) {
	if(buffer == 0) return;
...
bool cuda_buffer::fill(const device_queue& cqueue,
	...
	switch(pattern_size) {
		...
		default:
			...
			auto pattern_buffer = std::make_unique<uint8_t[]>(fill_size);
...
bool cuda_buffer::zero(const device_queue& cqueue) {
	if(buffer == 0) return false;
	static constexpr const uint32_t zero_pattern { 0u };
	return fill(cqueue, &zero_pattern, sizeof(zero_pattern), 0, 0);
}

void* __attribute__((aligned(128))) cuda_buffer::map(const device_queue& cqueue,
...
bool cuda_buffer::create_shared_vulkan_buffer(const bool copy_host_data) {
	...
	if (shared_vk_buffer == nullptr || cuda_vk_buffer != nullptr /* !nullptr if resize */ || !cuda_vk_sema) {
		...
		if (shared_vk_buffer == nullptr || cuda_vk_buffer != nullptr /* !nullptr if resize */) {
			...
			if (!copy_host_data) {
				shared_vk_buffer_flags |= MEMORY_FLAG::NO_INITIAL_COPY;
			}
...
#if !defined(FLOOR_NO_VULKAN)
bool cuda_buffer::acquire_vulkan_buffer(const device_queue* cqueue_, const vulkan_queue* vk_queue_) const {
	if (!vk_object_state) {
...
bool cuda_buffer::release_vulkan_buffer(const device_queue* cqueue_, const vulkan_queue*) const {
	if (vk_object_state) {
...
#else
bool cuda_buffer::acquire_vulkan_buffer(const device_queue*, const vulkan_queue*) const {
	return false;
}
bool cuda_buffer::release_vulkan_buffer(const device_queue*, const vulkan_queue*) const {
	return false;
}
#endif
