...
std::optional<device_info_t> query(id <MTLDevice> device) {
	@autoreleasepool {
		...
		const auto query_and_parse_gpu_config = [](std::unordered_map<std::string, std::function<void(uint32_t)>>&& property_cbs) {
			std::string ioreg_query;
			core::system("ioreg -r -k \"GPUConfigurationVariable\" | grep \"GPUConfigurationVariable\"", ioreg_query);
			if (!ioreg_query.empty() && ioreg_query.find("GPUConfigurationVariable") != std::string::npos) {
				auto props_start = ioreg_query.find("= {");
				auto props_end = ioreg_query.rfind('}');
				if (props_start != std::string::npos && props_end != std::string::npos && props_start + 3 < props_end) {
					auto props_tokens = core::tokenize(ioreg_query.substr(props_start + 3, (props_end - props_start) - 3), ',');
					const auto parse_prop_key_value = [](const std::string& prop) -> std::pair<std::string, uint32_t> {
						const auto quote_pos_front = prop.find('"');
						const auto quote_pos_back = prop.find('"', quote_pos_front + 1);
						const auto eq_pos = prop.find('=');
						if (quote_pos_front == std::string::npos || quote_pos_back == std::string::npos || eq_pos == std::string::npos) {
							return { "", 0u };
						}
		...
		} else if (dev_name.find("Apple") != std::string::npos) {
