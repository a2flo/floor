...
static MTL4RenderPassDescriptor* create_metal_render_pass_desc_from_description(const render_pass_description& desc,
																				const bool is_multi_view) {
	MTL4RenderPassDescriptor* pass_desc = [MTL4RenderPassDescriptor new];
	pass_desc.supportColorAttachmentMapping = false;
	...
	for (size_t i = 0, count = desc.attachments.size(); i < count; ++i) {
		...
		if (has_flag<IMAGE_TYPE::FLAG_DEPTH>(att.format)) {
			// depth attachment
			pass_desc.depthAttachment.loadAction = metal4_pass::metal_load_action_from_load_op(att.load_op);
			pass_desc.depthAttachment.storeAction = metal4_pass::metal_store_action_from_store_op(att.store_op);
			pass_desc.depthAttachment.clearDepth = double(att.clear.depth);
		} else {
			// color attachment
			pass_desc.colorAttachments[color_att_counter].loadAction = metal4_pass::metal_load_action_from_load_op(att.load_op);
			pass_desc.colorAttachments[color_att_counter].storeAction = metal4_pass::metal_store_action_from_store_op(att.store_op);
...
metal4_pass::metal4_pass(const render_pass_description& pass_desc_, const bool with_multi_view_support) :
...
MTLLoadAction metal4_pass::metal_load_action_from_load_op(const LOAD_OP& load_op) {
	switch (load_op) {
		case LOAD_OP::LOAD:
...
MTLStoreAction metal4_pass::metal_store_action_from_store_op(const STORE_OP& store_op) {
	switch (store_op) {
		case STORE_OP::STORE:
...
id <MTL4RenderCommandEncoder> metal4_pass::create_encoder(id <MTL4CommandBuffer> cmd_buffer,
