...
bool metal_renderer::commit_and_release(std::unique_ptr<graphics_renderer>&& renderer, completion_handler_f&& compl_handler) {
	return metal_renderer_internal::commit_and_release_internal(std::move(renderer), std::move(compl_handler));
}

bool metal_renderer::commit_and_release(std::shared_ptr<graphics_renderer>&& renderer, completion_handler_f&& compl_handler) {
	return metal_renderer_internal::commit_and_release_internal(std::move(renderer), std::move(compl_handler));
}
...
graphics_renderer::drawable_t* metal_renderer::get_next_drawable(const bool get_multi_view_drawable) {
	@autoreleasepool {
		...
		cur_drawable->valid = true;
		cur_drawable->metal_image = std::make_shared<metal_image>(cqueue, mtl_drawable.texture, std::span<uint8_t> {},
...
bool metal_renderer::set_attachments(std::vector<attachment_t>& attachments) {
...
bool metal_renderer::set_tessellation_factors(const device_buffer& tess_factors_buffer) {
...
#if defined(FLOOR_DEBUG)
			if (draw_entry->control_point_buffers.size() != desc.tessellation.vertex_attributes.size()) {
...
void metal_renderer::wait_for_fence(const device_fence& fence, const SYNC_STAGE before_stage) {
	@autoreleasepool {
		[encoder waitForFence:((const metal_fence&)fence).get_metal_fence() beforeStages:sync_stage_to_metal_render_stages(before_stage)];
	}
}

void metal_renderer::signal_fence(device_fence& fence, const SYNC_STAGE after_stage) {
	@autoreleasepool {
