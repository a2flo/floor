...
static auto get_external_symbol_ptr(const std::string& name) {
#if !defined(__WINDOWS__)
	return dlsym(
...
struct section_t {
	...
	void dump(std::ostream& sstr) const {
		sstr << "section: " << name;
		...
		sstr << ", 0x" << std::hex << std::uppercase << (uint64_t)header_ptr->flags << std::nouppercase << std::dec;
...
struct symbol_t {
	...
	void dump(std::ostream& sstr, const std::vector<section_t>& sections) const {
		sstr << "symbol " << name << ": ";
		...
		sstr << ", value/offset: " << std::to_string(symbol_ptr->value);
		sstr << ", size: " << std::to_string(symbol_ptr->size);
...
struct relocation_t {
	...
	void dump(std::ostream& sstr, const std::vector<section_t>& sections, const std::vector<symbol_t>& symbols) const {
		sstr << "reloc: symbol ";
		...
		sstr << ", type: " << (std::underlying_type_t<ELF_RELOCATION_TYPE_X86_64>)reloc_ptr->type_x86_64;
...
struct elf_binary::elf_info_t {
	...
	std::vector<const elf64_section_header_entry_t*> section_header_entries;
	...
	std::unordered_map<const section_t*, const uint8_t*> ro_section_map;
	
	//! contains all "internal" execution instances for this binary
	std::vector<internal_instance_t> instances;
...
elf_binary::elf_binary(const uint8_t* binary_data, const size_t binary_size_) : binary_size(binary_size_) {
	...
	binary = std::make_unique<uint8_t[]>(binary_size);
	memcpy(binary.get(), binary_data, binary_size);
	
	init_elf();
}

elf_binary::elf_binary(const std::string& file_name) {
...
const std::vector<std::string>& elf_binary::get_function_names() const {
	if (!info || !valid) {
...
#error "unhandled arch"
		...
		const std::string_view names { (const char*)&binary[section_name_header.offset], section_name_header.size };
		for (uint32_t i = 0; i < header.section_header_table_entry_count; ++i) {
			...
			section.name = std::string(names_start.substr(0, term_pos));
			...
			if (section.header_ptr->type == ELF_SECTION_TYPE::SYMBOL_TABLE) {
				...
				for (uint64_t sym_idx = 0, sym_count = section.header_ptr->size / sizeof(elf64_symbol_t); sym_idx < sym_count; ++sym_idx) {
					...
					if (sym_term_pos == std::string::npos) {
						log_error("didn't find symbol name terminator");
						return false;
					}
					sym.name = std::string(sym_names_start.substr(0, sym_term_pos));
		...
		for (const auto& section : info->sections) {
			...
			if (section.header_ptr->type == ELF_SECTION_TYPE::RELOCATION_ENTRIES_ADDEND) {
				...
				std::vector<relocation_t>* relocations = nullptr;
...
static bool map_memory(aligned_ptr<uint8_t>& mem,
					   ...
					   const std::string& primary_section_name) {
	// find all matching sections that need to be allocated:
	// section -> offset
	std::vector<std::pair<const section_t*, uint64_t>> alloc_sections;
	for (const auto& section : sections) {
		...
		if (has_required_flags && !has_prohibited_flags) {
			if (section.name == primary_section_name) {
				...
				alloc_sections.insert(alloc_sections.begin(), std::pair { &section, 0u });
			} else {
				alloc_sections.emplace_back(std::pair { &section, 0u });
			}
		}
	}
...
bool elf_binary::instantiate(const uint32_t instance_idx) {
	...
	for (const auto& sym : info->symbols) {
		...
		if (!sym.name.empty() && sym.symbol_ptr->section_header_table_index == 0 &&
			...
			std::cout << "\t-> external symbol ptr: 0x" << std::hex << std::uppercase << std::to_string(uint64_t(ext_sym_ptr)) << std::nouppercase << std::dec << std::endl;
		}
	}
#endif
	...
	// section -> offset
	std::vector<std::pair<const section_t*, uint64_t>> rw_sections;
	std::vector<std::pair<const section_t*, uint64_t>> exec_sections;
	for (const auto& section : info->sections) {
		if (!has_flag<ELF_SECTION_FLAG::ALLOCATE>(section.header_ptr->flags)) {
			continue;
		}
		...
		if (is_writable && !is_exec) {
			rw_sections.emplace_back(std::pair { &section, 0u });
		} else if (!is_writable && is_exec) {
			exec_sections.emplace_back(std::pair { &section, 0u });
		}
	}
	...
	for (const auto& sym : info->symbols) {
		...
		ext_instance.functions.emplace(std::string(sym.name), instance.exec_memory.get() + sym.symbol_ptr->value);
	}
