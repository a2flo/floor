...
bool vulkan_renderer::commit_internal(const bool is_blocking, const bool is_finishing,
	...
	if (!is_blocking && is_presenting) {
		(void)add_completion_handler([this]() {
			((vulkan_context&)ctx).queue_present(cqueue, *internal->cur_drawable->vk_drawable);
		});
	}
	...
	if (is_blocking) {
		...
		if (is_presenting) {
			((vulkan_context&)ctx).queue_present(cqueue, *cur_drawable->vk_drawable);
			is_presenting = false;
		}
...
graphics_renderer::drawable_t* vulkan_renderer::get_next_drawable(const bool get_multi_view_drawable) {
	auto drawable_ret = ((vulkan_context&)ctx).acquire_next_image(cqueue, get_multi_view_drawable);
