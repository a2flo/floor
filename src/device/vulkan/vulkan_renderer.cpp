...
struct vulkan_drawable_t final : public graphics_renderer::drawable_t {
	...
	std::unique_ptr<vulkan_drawable_image_info> vk_drawable;
	std::unique_ptr<vulkan_image_internal> vk_image;
	
	using graphics_renderer::drawable_t::valid;
};
...
struct vulkan_renderer_internal {
	...
	static bool commit_and_release_internal(smart_ptr_type<renderer_type>&& renderer, graphics_renderer::completion_handler_f&& compl_handler) {
		...
		auto queue_submission_compl_handler = [retained_renderer = std::shared_ptr<renderer_type>(std::move(renderer))](const vulkan_command_buffer&) {
...
vulkan_renderer::vulkan_renderer(const device_queue& cqueue_,
								 const graphics_pass& pass_,
								 const graphics_pipeline& pipeline_,
								 const bool multi_view_) :
graphics_renderer(cqueue_, pass_, pipeline_, multi_view_), internal(std::make_unique<vulkan_renderer_internal>()) {
	if (!valid) {
...
VkFramebuffer vulkan_renderer::create_vulkan_framebuffer(const VkRenderPass& vk_render_pass, const std::string& pass_debug_label [[maybe_unused]],
	...
	std::string debug_label = "framebuffer";
	if (!pass_debug_label.empty()) {
		debug_label += ":" + pass_debug_label;
	}
	set_vulkan_debug_label(vk_dev, VK_OBJECT_TYPE_FRAMEBUFFER, uint64_t(framebuffer), debug_label);
#endif
...
bool vulkan_renderer::begin(const dynamic_render_state_t dynamic_render_state) {
	...
	// if a drawable is associated with this renderer, we need to take care of some swapchain/present handling
	auto cur_drawable = internal->cur_drawable.get();
	if (cur_drawable) {
	...
	vkCmdSetViewport(render_cmd_buffer.cmd_buffer, 0, 1, &internal->cur_viewport);
	
	if (!dynamic_render_state.scissor) {
		internal->cur_render_area = {
	...
	if (uint32_t(internal->cur_render_area.offset.x) >= (uint32_t)internal->cur_viewport.width ||
		uint32_t(internal->cur_render_area.offset.y) >= (uint32_t)internal->cur_viewport.height) {
		log_error("scissor offset is out-of-bounds: $ >= $",
				  int2 { internal->cur_render_area.offset.x, internal->cur_render_area.offset.y },
				  float2 { internal->cur_viewport.width, internal->cur_viewport.height });
		return false;
	}
	if (dynamic_render_state.viewport) {
		// clamp scissor rect and render area if we have a dynamic viewport
		const auto clamped_width = std::min(uint32_t(internal->cur_render_area.offset.x) + internal->cur_render_area.extent.width,
											(uint32_t)internal->cur_viewport.width);
		const auto clamped_height = std::min(uint32_t(internal->cur_render_area.offset.y) + internal->cur_render_area.extent.height,
											 (uint32_t)internal->cur_viewport.height);
		internal->cur_render_area.extent.width = clamped_width - uint32_t(internal->cur_render_area.offset.x);
	...
	const VkRenderPassBeginInfo pass_begin_info {
		...
		.renderArea = internal->cur_render_area,
...
bool vulkan_renderer::commit_and_release(std::unique_ptr<graphics_renderer>&& renderer, completion_handler_f&& compl_handler) {
	return vulkan_renderer_internal::commit_and_release_internal(std::move(renderer), std::move(compl_handler));
}

bool vulkan_renderer::commit_and_release(std::shared_ptr<graphics_renderer>&& renderer, completion_handler_f&& compl_handler) {
	return vulkan_renderer_internal::commit_and_release_internal(std::move(renderer), std::move(compl_handler));
}
...
bool vulkan_renderer::commit_internal(const bool is_blocking, const bool is_finishing,
	...
	if (!is_blocking && is_presenting) {
		(void)add_completion_handler([this]() {
			((vulkan_context*)cqueue.get_device().context)->queue_present(cqueue, *internal->cur_drawable->vk_drawable);
		});
	}
	...
	if (is_presenting) {
		...
		cur_drawable->vk_image->transition(&cqueue, render_cmd_buffer.cmd_buffer, 0 /* as per doc */,
										   cur_drawable->vk_drawable->present_layout,
										   VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
	...
	if (is_blocking) {
		...
		if (is_presenting) {
			((vulkan_context*)cqueue.get_device().context)->queue_present(cqueue, *cur_drawable->vk_drawable);
			is_presenting = false;
		}
...
graphics_renderer::drawable_t* vulkan_renderer::get_next_drawable(const bool get_multi_view_drawable) {
	auto drawable_ret = ((vulkan_context*)cqueue.get_device().context)->acquire_next_image(cqueue, get_multi_view_drawable);
	if (!drawable_ret.first || !drawable_ret.second) {
		return nullptr;
	}
	
	internal->cur_drawable = std::make_unique<vulkan_drawable_t>();
	internal->cur_drawable->vk_drawable = std::move(drawable_ret.second);
	internal->cur_drawable->valid = true;
	const auto& vk_drawable = *internal->cur_drawable->vk_drawable;
...
void vulkan_renderer::present() {
	if (!internal->cur_drawable || !internal->cur_drawable->is_valid()) {
...
bool vulkan_renderer::set_attachments(std::vector<attachment_t>& attachments) {
...
bool vulkan_renderer::set_attachment(const uint32_t& index, attachment_t& attachment) {
	...
	if (internal->cur_drawable && internal->cur_drawable->image == attachment.image) {
		return true;
	}
...
void vulkan_renderer::signal_fence(device_fence& fence, const SYNC_STAGE after_stage) {
