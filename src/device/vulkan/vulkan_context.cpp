...
#include "internal/vulkan_drawable.hpp"
...
struct vulkan_context_internal {
	struct screen_data {
		...
		std::vector<VkImageLayout> swapchain_prev_layouts;
		const vulkan_device* render_device { nullptr };
		bool x11_forwarding { false };
		std::shared_ptr<vulkan_image_internal> x11_screen;
		std::optional<VkHdrMetadataEXT> hdr_metadata;
	...
	struct vr_screen_data {
		...
		VkColorSpaceKHR color_space { VK_COLOR_SPACE_SRGB_NONLINEAR_KHR };
		IMAGE_TYPE image_type { IMAGE_TYPE::NONE };
		std::vector<std::shared_ptr<device_image>> images;
		std::vector<device_image*> external_swapchain_images;
		std::vector<atomic_spin_lock> image_locks;
		const vulkan_device* render_device { nullptr };
		bool has_wide_gamut { false };
	} vr_screen;
...
static uint32_t aftermath_shader_debug_info(const void* shader_debug_info, const uint32_t shader_debug_info_size, void*) {
	if (shader_debug_info && shader_debug_info_size > 0) {
		static std::atomic<uint32_t> counter = 0;
		file_io::buffer_to_file("aftermath_shader_debug_info_" + floor::get_app_name() + "_" + std::to_string(counter++) + ".nvdbg",
								(const char*)shader_debug_info, shader_debug_info_size);
	}
	return 0;
}
static bool setup_nvidia_aftermath(vulkan_context* ctx) {
	static std::atomic<bool> aftermath_init_successful { false };
	static std::once_flag did_setup;
	std::call_once(did_setup, [&ctx]() {
#if defined(__WINDOWS__)
...
static device_mem_info_t handle_and_select_device_memory(const VkPhysicalDevice& dev, const decltype(VkPhysicalDeviceProperties::deviceName)& dev_name) {
	...
	ret.mem_props = std::make_shared<VkPhysicalDeviceMemoryProperties>();
...
vulkan_context::vulkan_context(const DEVICE_CONTEXT_FLAGS ctx_flags, const bool has_toolchain_,
...
enable_renderer(enable_renderer_) {
	...
	std::vector<const char*> instance_extensions_ptrs;
	std::string inst_exts_str, inst_layers_str;
	...
	if (floor::get_vulkan_validation()) {
		...
		VK_CALL_RET(vkCreateDebugUtilsMessengerEXT(ctx, &debug_messenger_info, nullptr, &internal->debug_utils_messenger),
	...
	std::vector<VkPhysicalDevice> queried_devices(device_count);
	...
	auto gpu_counter = (std::underlying_type_t<device::TYPE>)device::TYPE::GPU0;
	auto cpu_counter = (std::underlying_type_t<device::TYPE>)device::TYPE::CPU0;
	...
	for (const auto& phys_dev : queried_devices) {
		...
		std::vector<VkExtensionProperties> supported_dev_exts(dev_ext_count);
		vkEnumerateDeviceExtensionProperties(phys_dev, nullptr, &dev_ext_count, supported_dev_exts.data());
		std::set<std::string> device_supported_extensions_set;
		std::set<std::string> device_extensions_set;
		static constexpr const std::array filtered_exts {
		...
		for (const auto& ext : supported_dev_exts) {
			...
			if (!enable_renderer || internal->screen.x11_forwarding) {
				if (ext_name.find("VK_KHR_swapchain" /* _* */) != std::string::npos) {
					continue;
				}
			}
			if (is_filtered) {
		...
		//
		std::vector<VkQueueFamilyProperties> dev_queue_family_props(queue_family_count);
		...
		for(uint32_t i = 0; i < queue_family_count; ++i) {
			max_queue_count = std::max(max_queue_count, dev_queue_family_props[i].queueCount);
		}
		if(max_queue_count == 0) {
			log_error("device $ supports no queues", props.deviceName);
			continue;
		}
		const std::vector<float> priorities(max_queue_count, 0.0f);
		
		std::vector<VkDeviceQueueCreateInfo> queue_create_info(queue_family_count);
		for(uint32_t i = 0; i < queue_family_count; ++i) {
		...
		if (vr_ctx) {
			...
			if (!vr_dev_exts.empty()) {
				...
				for (const auto& ext_str : vr_dev_ext_strs) {
					bool is_filtered = false;
					for (const auto& filtered_ext : filtered_exts) {
						if (ext_str.find(filtered_ext) != std::string::npos) {
							is_filtered = true;
							break;
						}
					}
		...
		std::string dev_exts_str, dev_layers_str;
		...
		std::vector<const char*> device_extensions_ptrs;
		for(const auto& ext : device_extensions) {
		...
		// add device
		devices.emplace_back(std::make_unique<vulkan_device>());
		auto& device = (vulkan_device&)*devices.back();
		physical_devices.emplace_back(phys_dev);
		logical_devices.emplace_back(dev);
		device.vk = std::make_shared<VolkDeviceTable>();
		volkLoadDeviceTable(device.vk.get(), dev);
		device.context = this;
		...
		std::copy_n(std::begin(vulkan11_props.deviceUUID), device.uuid.size(), std::begin(device.uuid));
		device.has_uuid = true;
		device.platform_vendor = VENDOR::KHRONOS; // not sure what to set here
		device.version_str = (std::to_string(VK_VERSION_MAJOR(props.apiVersion)) + "." +
							  std::to_string(VK_VERSION_MINOR(props.apiVersion)) + "." +
							  std::to_string(VK_VERSION_PATCH(props.apiVersion)));
		device.driver_version_str = std::to_string(props.driverVersion);
		device.conformance_version = (std::to_string(vulkan12_props.conformanceVersion.major) + "." +
									  std::to_string(vulkan12_props.conformanceVersion.minor) + "." +
									  std::to_string(vulkan12_props.conformanceVersion.subminor) + "." +
									  std::to_string(vulkan12_props.conformanceVersion.patch));
		...
		if(props.vendorID < 0x10000) {
			switch(props.vendorID) {
				case 0x1002:
					...
					device.driver_version_str = std::to_string(VK_VERSION_MAJOR(props.driverVersion)) + ".";
					device.driver_version_str += std::to_string(VK_VERSION_MINOR(props.driverVersion)) + ".";
					device.driver_version_str += std::to_string(VK_VERSION_PATCH(props.driverVersion));
					break;
				case 0x10DE:
					device.vendor = VENDOR::NVIDIA;
					device.vendor_name = "NVIDIA";
					device.driver_version_str = std::to_string((props.driverVersion >> 22u) & 0x3FF) + ".";
					device.driver_version_str += std::to_string((props.driverVersion >> 14u) & 0xFF) + ".";
					device.driver_version_str += std::to_string((props.driverVersion >> 6u) & 0xFF);
					break;
				case 0x8086:
		...
		switch(props.deviceType) {
			...
			case VK_PHYSICAL_DEVICE_TYPE_CPU:
				device.type = (device::TYPE)cpu_counter;
				++cpu_counter;
		...
		for (uint32_t i = 0; i < queue_family_count; ++i) {
			...
			static constexpr const VkQueueFlags device_queue_flags { VK_QUEUE_COMPUTE_BIT | VK_QUEUE_TRANSFER_BIT };
			...
			if (((dev_queue_family_props[i].queueFlags & device_queue_flags) == device_queue_flags) &&
	...
	if (enable_renderer) {
		...
		resize_handler_fnctr = std::bind(&vulkan_context::resize_handler, this, std::placeholders::_1, std::placeholders::_2);
...
#if defined(FLOOR_DEBUG)
	if (vkDestroyDebugUtilsMessengerEXT != nullptr && internal->debug_utils_messenger != nullptr) {
		vkDestroyDebugUtilsMessengerEXT(ctx, internal->debug_utils_messenger, nullptr);
	}
#endif
	...
	if (!internal->screen.x11_forwarding) {
		...
		if (!reset_present_fences) {
			...
			for (auto& fence : present_fences) {
				vkDestroyFence(internal->screen.render_device->device, fence, nullptr);
			}
			present_fences.clear();
		} else if (!present_fences.empty()) {
			// -> only reset
			vkResetFences(internal->screen.render_device->device, uint32_t(present_fences.size()), present_fences.data());
		}
		next_sema_index = 0;
		semas_in_use.reset();
		...
		if (!internal->screen.swapchain_image_views.empty()) {
			for (auto& image_view : internal->screen.swapchain_image_views) {
				vkDestroyImageView(internal->screen.render_device->device, image_view, nullptr);
			}
		}
		
		if (internal->screen.swapchain != nullptr) {
			vkDestroySwapchainKHR(internal->screen.render_device->device, internal->screen.swapchain, nullptr);
		}
		
		if (internal->screen.surface != nullptr) {
			vkDestroySurfaceKHR(ctx, internal->screen.surface, nullptr);
		}
	}
...
bool vulkan_context::reinit_renderer(const uint2 screen_size) {
	...
	if (screen.x11_forwarding) {
		...
		if (const auto screen_img_type = image_type_from_vulkan_format(screen.format); screen_img_type) {
			screen.image_type = *screen_img_type;
		} else {
			log_error("invalid screen format");
			return false;
		}
		screen.x11_screen = static_pointer_cast<vulkan_image_internal>(create_image(vk_queue, screen.size,
																					IMAGE_TYPE::IMAGE_2D |
																					IMAGE_TYPE::BGRA8UI_NORM |
	...
	if (!want_wide_color) {
		...
		const auto img_type = image_type_from_vulkan_format(screen.format);
		log_debug("using screen format $ ($X)", device_image::image_type_to_string(*img_type), screen.format);
	} else {
		...
		static constexpr const std::array wide_color_tiers {
		...
		if (found_format) {
			// if we get here, we know that the format is also representable by IMAGE_TYPE
			const auto img_type = image_type_from_vulkan_format(screen.format);
			log_debug("wide-gamut enabled (using format $ ($X))", device_image::image_type_to_string(*img_type), screen.format);
	...
	const auto screen_img_type = image_type_from_vulkan_format(screen.format);
	if (!screen_img_type) {
	...
	if (floor::get_hdr() && hdr_supported) {
		if (!screen.has_wide_gamut) {
			...
			std::string color_space_name = "unknown";
			switch (screen.color_space) {
				case VK_COLOR_SPACE_SRGB_NONLINEAR_KHR:
	...
	auto swapchain_image_count = (surface_caps.maxImageCount > 0 ?
								  std::min(surface_caps.maxImageCount, max_swapchain_image_count) :
...
bool vulkan_context::init_vr_renderer() {
	auto& screen = internal->screen;
	auto& vr_screen = internal->vr_screen;
	...
	if (!vr_ctx->has_swapchain()) {
		...
		vr_screen.image_type = image_type_from_vulkan_format(vr_screen.format).value_or(IMAGE_TYPE::NONE);
		...
		if (!has_flag<IMAGE_TYPE::FLAG_ARRAY>(vr_screen.image_type)) {
...
std::pair<bool, std::unique_ptr<vulkan_drawable_image_info>> vulkan_context::acquire_next_vr_image(const device_queue& dev_queue) NO_THREAD_SAFETY_ANALYSIS {
	auto& vr_screen = internal->vr_screen;
	...
	if (vr_ctx->has_swapchain()) {
		...
		auto vk_swapchain_image = (vulkan_image_internal*)swapchain_image;
		vr_screen.external_swapchain_images[vr_screen.image_index] = swapchain_image;
		return {
			true,
			std::make_unique<vulkan_drawable_image_info>(vulkan_drawable_image_info {
		...
		auto vr_image = (vulkan_image_internal*)vr_screen.images[vr_screen.image_index].get();
		...
		return {
			...
			std::make_unique<vulkan_drawable_image_info>(vulkan_drawable_image_info {
				...
				.base_type = IMAGE_TYPE::IMAGE_2D_ARRAY,
			})
		};
	}
}

std::pair<bool, std::unique_ptr<vulkan_drawable_image_info>> vulkan_context::acquire_next_image(const device_queue& dev_queue,
	...
	//       -> use a separate sema index (and allocate a conservative amount of acquisition fences/semas)
	...
	set_vulkan_debug_label(*screen.render_device, VK_OBJECT_TYPE_IMAGE, uint64_t(screen.swapchain_images[screen.image_index]),
						   "swapchain_image#" + std::to_string(screen.image_index));
#endif
...
bool vulkan_context::present_image(const device_queue& dev_queue, const vulkan_drawable_image_info& drawable) {
	auto& screen = internal->screen;
...
bool vulkan_context::queue_present(const device_queue& dev_queue, const vulkan_drawable_image_info& drawable) NO_THREAD_SAFETY_ANALYSIS {
	auto& screen = internal->screen;
	
	if (vr_ctx && drawable.layer_count > 1) {
#if !defined(FLOOR_NO_OPENVR) || !defined(FLOOR_NO_OPENXR)
		device_image* present_image = nullptr;
		if (vr_ctx->has_swapchain()) {
			present_image = internal->vr_screen.external_swapchain_images[drawable.index];
		} else {
			present_image = internal->vr_screen.images[drawable.index].get();

			// keep image state in sync
			((vulkan_image_internal*)present_image)->update_with_external_vulkan_state(drawable.present_layout, VK_ACCESS_2_TRANSFER_READ_BIT);
		}
		...
		// unlock image again
		internal->vr_screen.image_locks[drawable.index].unlock();
#endif
	} else {
		const auto& vk_queue = (const vulkan_queue&)dev_queue;
...
const device_queue* vulkan_context::get_device_default_compute_queue(const device& dev) const {
	...
	for (auto&& default_queue : default_device_queues) {
		if (default_queue.first == &dev) {
			return default_queue.second.get();
		}
	}
...
std::optional<uint32_t> vulkan_context::get_max_distinct_queue_count(const device& dev) const {
	const auto& vk_dev = (const vulkan_device&)dev;
	return vk_dev.queue_counts[vk_dev.all_queue_family_index];
}

std::optional<uint32_t> vulkan_context::get_max_distinct_compute_queue_count(const device& dev) const {
	const auto& vk_dev = (const vulkan_device&)dev;
...
std::shared_ptr<device_program> vulkan_context::add_universal_binary(const std::string& file_name) {
...
std::shared_ptr<device_program> vulkan_context::add_universal_binary(const std::span<const uint8_t> data) {
...
std::shared_ptr<vulkan_program> vulkan_context::add_program(vulkan_program::program_map_type&& prog_map) {
	...
	auto prog = std::make_shared<vulkan_program>(std::move(prog_map));
	{
		GUARD(programs_lock);
...
std::shared_ptr<device_program> vulkan_context::add_program_file(const std::string& file_name,
...
std::shared_ptr<device_program> vulkan_context::add_program_source(const std::string& source_code,
															   const std::string additional_options) {
	compile_options options { .cli = additional_options };
...
void vulkan_context::create_fixed_sampler_set() const {
	...
	for (auto& dev : devices) {
		...
		std::vector<VkDescriptorSetLayoutBinding> fixed_sampler_bindings(max_sampler_combinations);
...
std::unique_ptr<graphics_pipeline> vulkan_context::create_graphics_pipeline(const render_pipeline_description& pipeline_desc,
																	   const bool with_multi_view_support) const {
	auto pipeline = std::make_unique<vulkan_pipeline>(pipeline_desc, devices, with_multi_view_support && (vr_ctx != nullptr));
...
std::unique_ptr<graphics_pass> vulkan_context::create_graphics_pass(const render_pass_description& pass_desc,
															   const bool with_multi_view_support) const {
	auto pass = std::make_unique<vulkan_pass>(pass_desc, devices, with_multi_view_support && (vr_ctx != nullptr));
...
std::unique_ptr<graphics_renderer> vulkan_context::create_graphics_renderer(const device_queue& cqueue,
	...
	auto renderer = std::make_unique<vulkan_renderer>(cqueue, pass, pipeline, create_multi_view_renderer);
...
IMAGE_TYPE vulkan_context::get_renderer_image_type() const {
	return (!vr_ctx ? internal->screen.image_type : internal->vr_screen.image_type);
}

uint4 vulkan_context::get_renderer_image_dim() const {
	if (!vr_ctx) {
		return { internal->screen.size, 0, 0 };
	} else {
		return { internal->vr_screen.size, internal->vr_screen.layer_count, 0 };
	}
}

bool vulkan_context::is_vr_supported() const {
	return (vr_ctx ? true : false);
}

vr_context* vulkan_context::get_renderer_vr_context() const {
	return vr_ctx;
}

void vulkan_context::set_hdr_metadata(const hdr_metadata_t& hdr_metadata_) {
	...
	set_vk_screen_hdr_metadata();
	if (hdr_supported && internal->screen.hdr_metadata) {
		vkSetHdrMetadataEXT(internal->screen.render_device->device, 1, &internal->screen.swapchain, &*internal->screen.hdr_metadata);
	}
}

void vulkan_context::set_vk_screen_hdr_metadata() {
	if (!internal->screen.hdr_metadata) {
		return;
	}
	
	internal->screen.hdr_metadata = {
