...
static bool create_vulkan_pipeline(vulkan_pipeline_state_t& state,
	...
	if (!pipeline_desc.debug_label.empty()) {
		set_vulkan_debug_label(vk_dev, VK_OBJECT_TYPE_PIPELINE_LAYOUT, uint64_t(state.layout), "layout:" + pipeline_desc.debug_label);
	}
#endif
	...
	const VkPipelineMultisampleStateCreateInfo multisample_state {
		...
		.pNext = nullptr,
		.flags = 0,
		.rasterizationSamples = sample_count_to_vulkan_sample_count(pipeline_desc.sample_count),
	...
	const bool has_depth_attachment = (pipeline_desc.depth_attachment.format != IMAGE_TYPE::NONE);
	...
	const bool log_binary = (vulkan_function::should_log_vulkan_binary(vk_vs_entry->info->name) ||
...
#if defined(FLOOR_DEBUG)
	if (!pipeline_desc.debug_label.empty()) {
		set_vulkan_debug_label(vk_dev, VK_OBJECT_TYPE_PIPELINE, uint64_t(state.pipeline), pipeline_desc.debug_label);
	}
#endif
...
const vulkan_pipeline_state_t* vulkan_pipeline::get_vulkan_pipeline_state(const device& dev,
