...
static property_type get_vr_property(vr::IVRSystem& system,
	...
	if constexpr (std::is_same_v<property_type, std::string>) {
	...
	} else if constexpr (std::is_same_v<property_type, bool>) {
		return system.GetBoolTrackedDeviceProperty(idx, prop, nullptr);
	} else if constexpr (std::is_same_v<property_type, float>) {
		return system.GetFloatTrackedDeviceProperty(idx, prop, nullptr);
	} else if constexpr (std::is_same_v<property_type, int32_t>) {
		return system.GetInt32TrackedDeviceProperty(idx, prop, nullptr);
	} else if constexpr (std::is_same_v<property_type, uint64_t>) {
...
static std::optional<vr_context::POSE_TYPE> tracker_type_from_name(const std::string& name) {
...
openvr_context::openvr_context() : vr_context() {
	...
	ctx = std::make_unique<vr::COpenVRContext>();
	...
	hmd_name = get_vr_property<std::string>(*system, vr::Prop_ModelNumber_String);
	vendor_name = get_vr_property<std::string>(*system, vr::Prop_ManufacturerName_String);
...
std::string openvr_context::get_vulkan_instance_extensions() const {
	if (compositor == nullptr) {
		return {};
	}
	...
	std::string vk_instance_exts(vk_instance_exts_len, '\0');
	compositor->GetVulkanInstanceExtensionsRequired(vk_instance_exts.data(), vk_instance_exts_len);
	return vk_instance_exts;
}

std::string openvr_context::get_vulkan_device_extensions(VkPhysicalDevice_T* physical_device) const {
	if (compositor == nullptr) {
		return {};
	}
	...
	std::string vk_dev_exts(vk_dev_exts_len, '\0');
	compositor->GetVulkanDeviceExtensionsRequired(physical_device, vk_dev_exts.data(), vk_dev_exts_len);
	return vk_dev_exts;
}

std::vector<std::shared_ptr<event_object>> openvr_context::handle_input() REQUIRES(!pose_state_lock) {
	...
	std::array<std::array<pose_t, handled_bone_count>, 2> hand_bone_poses {};
			...
			switch (action.second.type) {
					...
					if (data.bActive && data.bChanged) {
						switch (action.second.event_type) {
							case EVENT_TYPE::VR_APP_MENU_PRESS:
								events.emplace_back(std::make_shared<vr_app_menu_press_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_APP_MENU_TOUCH:
								events.emplace_back(std::make_shared<vr_app_menu_touch_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_MAIN_PRESS:
								events.emplace_back(std::make_shared<vr_main_press_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_MAIN_TOUCH:
								events.emplace_back(std::make_shared<vr_main_touch_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_SYSTEM_PRESS:
								events.emplace_back(std::make_shared<vr_system_press_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_SYSTEM_TOUCH:
								events.emplace_back(std::make_shared<vr_system_touch_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_TRACKPAD_PRESS:
								events.emplace_back(std::make_shared<vr_trackpad_press_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_TRACKPAD_TOUCH:
								events.emplace_back(std::make_shared<vr_trackpad_touch_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_THUMBSTICK_PRESS:
								events.emplace_back(std::make_shared<vr_thumbstick_press_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_THUMBSTICK_TOUCH:
								events.emplace_back(std::make_shared<vr_thumbstick_touch_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_TRIGGER_PRESS:
								events.emplace_back(std::make_shared<vr_trigger_press_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_TRIGGER_TOUCH:
								events.emplace_back(std::make_shared<vr_trigger_touch_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_GRIP_PRESS:
								events.emplace_back(std::make_shared<vr_grip_press_event>(cur_time, action.second.side, data.bState));
								break;
							case EVENT_TYPE::VR_GRIP_TOUCH:
								events.emplace_back(std::make_shared<vr_grip_touch_event>(cur_time, action.second.side, data.bState));
								break;
							default:
					...
					if (data.bActive && (data.deltaX != 0.0f || data.deltaY != 0.0f || data.deltaZ != 0.0f)) {
						switch (action.second.event_type) {
							case EVENT_TYPE::VR_TRACKPAD_MOVE:
								events.emplace_back(std::make_shared<vr_trackpad_move_event>(cur_time, action.second.side,
																						float2{ data.x, data.y },
																						float2{ data.deltaX, data.deltaY }));
								break;
							case EVENT_TYPE::VR_THUMBSTICK_MOVE:
								events.emplace_back(std::make_shared<vr_thumbstick_move_event>(cur_time, action.second.side,
																						  float2{ data.x, data.y },
																						  float2{ data.deltaX, data.deltaY }));
								break;
							case EVENT_TYPE::VR_TRIGGER_PULL:
								events.emplace_back(std::make_shared<vr_trigger_pull_event>(cur_time, action.second.side, data.x, data.deltaX));
								break;
							case EVENT_TYPE::VR_GRIP_PULL:
								events.emplace_back(std::make_shared<vr_grip_pull_event>(cur_time, action.second.side, data.x, data.deltaX));
								break;
							case EVENT_TYPE::VR_TRACKPAD_FORCE:
								events.emplace_back(std::make_shared<vr_trackpad_force_event>(cur_time, action.second.side, data.x, data.deltaX));
								break;
							case EVENT_TYPE::VR_GRIP_FORCE:
								events.emplace_back(std::make_shared<vr_grip_force_event>(cur_time, action.second.side, data.x, data.deltaX));
								break;
							default:
								log_error("unknown/unhandled VR event: $", action.first);
								break;
				...
				case ACTION_TYPE::SKELETAL: {
					...
					if (data.bActive) {
						...
						std::vector<vr::VRBoneTransform_t> bone_transforms(expected_bone_count);
	...
	std::array<vr::TrackedDevicePose_t, vr::k_unMaxTrackedDeviceCount> vr_poses {};
				...
				switch (pose.type) {
					...
					case vr_context::POSE_TYPE::HAND_JOINT_PALM_LEFT...vr_context::POSE_TYPE::HAND_FOREARM_JOINT_ELBOW_RIGHT:
						...
						pose.angular_velocity = {
							vr_pose.vAngularVelocity.v[0],
							vr_pose.vAngularVelocity.v[1],
	...
	switch (dev_class) {
		...
		case vr::TrackedDeviceClass_GenericTracker: {
			...
			if (floor::get_vr_trackers()) {
				const auto tracker_name = get_vr_property<std::string>(*system, vr::Prop_ControllerType_String, idx);
...
bool openvr_context::present(const device_queue& cqueue, const device_image* image) {
	if (!image) {
	...
	const auto& vk_image = *(const vulkan_image_internal*)image;
	...
	vr::VRVulkanTextureData_t vr_vk_image {
		...
		.m_pInstance = ((const vulkan_context*)vk_dev.context)->get_vulkan_context(),
...
matrix4f openvr_context::get_eye_matrix(const VR_EYE& eye) const {
	...
	return matrix4f(mat.m[0][0], mat.m[1][0], mat.m[2][0], 0.0f,
					mat.m[0][1], mat.m[1][1], mat.m[2][1], 0.0f,
					mat.m[0][2], mat.m[1][2], mat.m[2][2], 0.0f,
					mat.m[0][3], mat.m[1][3], mat.m[2][3], 1.0f).inverted();
}
...
std::vector<vr_context::pose_t> openvr_context::get_pose_state() const REQUIRES(!pose_state_lock) {
	GUARD(pose_state_lock);
	return pose_state;
}
...
void openvr_context::update_hand_controller_types() REQUIRES(!device_type_map_lock) {
	...
	for (uint32_t hand_idx = 0; hand_idx < 2; ++hand_idx) {
		...
		const auto controller_name = get_vr_property<std::string>(*system, vr::Prop_ControllerType_String, hand_device_indices[hand_idx]);
		static const std::unordered_map<std::string, CONTROLLER_TYPE> controller_name_map {
