...
#include "device/vulkan/internal/vulkan_debug.hpp"
...
static XRAPI_ATTR XrBool32 XRAPI_CALL openxr_debug_callback(XrDebugUtilsMessageSeverityFlagsEXT severity,
	...
	if (cb_data != nullptr) {
		...
		if (cb_data->functionName) {
			debug_message += " in " + std::string(cb_data->functionName);
		}
		debug_message += "\n";
		...
		if (cb_data->objectCount > 0 && cb_data->objects != nullptr) {
			...
			for (uint32_t oidx = 0; oidx < cb_data->objectCount; ++oidx) {
				...
				debug_message += std::to_string(cb_data->objects[oidx].objectHandle) + ")\n";
			}
		}
	} else {
...
openxr_context::openxr_context() : vr_context(), internal(std::make_shared<openxr_context_internal>()) {
	platform = VR_PLATFORM::OPENXR;
	
	// query supported extensions and layers
	const auto query_extensions = [](const char* layer_name) -> std::unordered_map<std::string, XrExtensionProperties> {
		...
		std::vector<XrExtensionProperties> extensions_vec(ext_count, { .type = XR_TYPE_EXTENSION_PROPERTIES, .next = nullptr });
		...
		std::unordered_map<std::string, XrExtensionProperties> extensions;
	...
	const auto supports_extension = [&supported_extensions](const std::string extension, const uint32_t version = 0) {
		if (version == 0) {
	...
	const auto supports_layer = [&layers](const std::string_view layer_name) {
	...
	if (supports_extension(XR_FB_DISPLAY_REFRESH_RATE_EXTENSION_NAME)) {
		...
		internal->can_query_display_refresh_rate = true;
	}
	...
	// init VR
	std::string used_layers, used_extensions;
	...
	const auto app_name_len = std::min(floor::get_app_name().size(), size_t(XR_MAX_APPLICATION_NAME_SIZE - 1));
	...
	if (enable_debug_utils_callback) {
		XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrCreateDebugUtilsMessengerEXT",
										  reinterpret_cast<PFN_xrVoidFunction*>(&internal->create_debug_utils_messenger)),
		...
		XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrDestroyDebugUtilsMessengerEXT",
										  reinterpret_cast<PFN_xrVoidFunction*>(&internal->destroy_debug_utils_messenger)),
		...
		XR_CALL_RET(internal->create_debug_utils_messenger(instance, &create_info, &internal->debug_utils_messenger),
					"failed to create debug utils messenger");
	}
#endif
	
	// only PimaxXR has a known good Vulkan implementation
	if (std::string_view(instance_props.runtimeName).find("PimaxXR") != std::string::npos) {
	...
	internal->view_configs.resize(view_config_views_count, { .type = XR_TYPE_VIEW_CONFIGURATION_VIEW, .next = nullptr });
	XR_CALL_RET(xrEnumerateViewConfigurationViews(instance, system_id, XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
												  view_config_views_count, &view_config_views_count, internal->view_configs.data()),
				"failed to retrieve view configuration views");
	for (const auto& view_config : internal->view_configs) {
	...
	if (internal->view_configs.size() != 2) {
		log_error("OpenXR: expected 2 views");
		return;
	}
	recommended_render_size = {
		internal->view_configs[0].recommendedImageRectWidth,
		internal->view_configs[0].recommendedImageRectHeight,
	};
	...
	internal->views.resize(internal->view_configs.size(), { .type = XR_TYPE_VIEW, .next = nullptr });
	view_states.resize(internal->view_configs.size());
	
	// query extension function pointers
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrCreateVulkanInstanceKHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->CreateVulkanInstanceKHR)),
	...
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrCreateVulkanDeviceKHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->CreateVulkanDeviceKHR)),
	...
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrGetVulkanGraphicsDevice2KHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->GetVulkanGraphicsDevice2KHR)),
	...
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrGetVulkanGraphicsRequirements2KHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->GetVulkanGraphicsRequirements2KHR)),
	...
	internal->win_perf_counter_freq = SDL_GetPerformanceFrequency();
	internal->win_start_perf_counter = sdl_perf_counter - (sdl_ticks * internal->win_perf_counter_freq) / 1000ull;
	
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrConvertWin32PerformanceCounterToTimeKHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->ConvertWin32PerformanceCounterToTimeKHR)),
	...
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrConvertTimeToWin32PerformanceCounterKHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->ConvertTimeToWin32PerformanceCounterKHR)),
	...
	assert(SDL_GetPerformanceFrequency() == internal->unix_perf_counter_freq);
	internal->unix_start_time = sdl_perf_counter - sdl_ticks * 1000ull; // -> in ns
	
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrConvertTimespecTimeToTimeKHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->ConvertTimespecTimeToTimeKHR)),
	...
	XR_CALL_RET(xrGetInstanceProcAddr(instance, "xrConvertTimeToTimespecTimeKHR",
									  reinterpret_cast<PFN_xrVoidFunction*>(&internal->ConvertTimeToTimespecTimeKHR)),
...
openxr_context::~openxr_context() {
	if (internal->swapchain.swapchain) {
		XR_CALL_IGNORE(xrDestroySwapchain(internal->swapchain.swapchain), "failed to destroy OpenXR swapchain");
	}
	if (vk_dev) {
		for (auto& image_view : internal->swapchain.swapchain_vk_image_views) {
			if (image_view) {
	...
	for (uint32_t hand_idx = 0; hand_idx < 2; ++hand_idx) {
		if (hand_trackers[hand_idx]) {
			XR_CALL_IGNORE(internal->DestroyHandTracker(hand_trackers[hand_idx]), "failed to destroy OpenXR hand tracker");
		}
...
#if defined(FLOOR_DEBUG)
	if (internal->debug_utils_messenger) {
		XR_CALL_IGNORE(internal->destroy_debug_utils_messenger(internal->debug_utils_messenger),
...
int openxr_context::create_vulkan_instance(const VkInstanceCreateInfo& vk_create_info, VkInstance& vk_instance) {
	// check Vulkan requirements (need to do this, but don't care)
	XrGraphicsRequirementsVulkanKHR vk_reqs {
		.type = XR_TYPE_GRAPHICS_REQUIREMENTS_VULKAN_KHR,
		.next = nullptr,
	};
	XR_CALL_RET(internal->GetVulkanGraphicsRequirements2KHR(instance, system_id, &vk_reqs),
	...
	XR_CALL_RET(internal->CreateVulkanInstanceKHR(instance, &create_info, &vk_instance, &vk_result),
				"failed to create Vulkan instance for OpenXR", vk_result);
	return VK_SUCCESS;
}

int openxr_context::create_vulkan_device(const VkDeviceCreateInfo& vk_create_info, VkDevice& vk_dev_,
	...
	XR_CALL_RET(internal->GetVulkanGraphicsDevice2KHR(instance, &graphics_dev_info, &graphics_dev),
	...
	XR_CALL_RET(internal->CreateVulkanDeviceKHR(instance, &create_info, &vk_dev_, &vk_result),
				"failed to create Vulkan device for OpenXR", vk_result);
	return VK_SUCCESS;
	
}

bool openxr_context::create_session(vulkan_context& vk_ctx_, const vulkan_device& vk_dev_, const vulkan_queue& vk_queue) {
	vk_ctx = &vk_ctx_;
	vk_dev = &vk_dev_;
	...
	while (internal->can_query_display_refresh_rate) {
		XR_CALL_BREAK(internal->GetDisplayRefreshRateFB(session, &display_frequency),
	...
	internal->spaces.resize(space_count, XR_REFERENCE_SPACE_TYPE_MAX_ENUM);
	XR_CALL_RET(xrEnumerateReferenceSpaces(session, space_count, &space_count, internal->spaces.data()),
				"failed to query spaces", false);
	std::string space_names;
	bool has_stage_space = false, has_view_space = false;
	for (const auto& space : internal->spaces) {
		switch (space) {
			...
			case XR_REFERENCE_SPACE_TYPE_LOCAL_FLOOR_EXT:
				...
				break;
			default:
				space_names += "<unknown: " + std::to_string(space) + ">";
				break;
		}
		space_names += " ";
	}
	...
	const XrSwapchainCreateInfo swapchain_create_info{
		...
		.faceCount = 1,
		.arraySize = uint32_t(internal->views.size()),
		.mipCount = 1,
	};
	XR_CALL_RET(xrCreateSwapchain(session, &swapchain_create_info, &internal->swapchain.swapchain),
				"failed to create swapchain", false);
	
	uint32_t swapchain_image_count = 0;
	XR_CALL_RET(xrEnumerateSwapchainImages(internal->swapchain.swapchain, 0, &swapchain_image_count, nullptr),
				"failed to retrieve swapchain image count", false);
	
	log_msg("OpenXR: swapchain image count: $", swapchain_image_count);
	internal->swapchain.swapchain_vk_images.resize(swapchain_image_count);
	for (uint32_t swapchain_image_idx = 0; swapchain_image_idx < swapchain_image_count; ++swapchain_image_idx) {
		internal->swapchain.swapchain_vk_images[swapchain_image_idx] = {
	...
	XR_CALL_RET(xrEnumerateSwapchainImages(internal->swapchain.swapchain, swapchain_image_count, &swapchain_image_count,
										   reinterpret_cast<XrSwapchainImageBaseHeader*>(internal->swapchain.swapchain_vk_images.data())),
				"failed to enumerate swapchain images", false);
	
	// wrap swapchain images
	internal->swapchain.swapchain_images.resize(swapchain_image_count);
	internal->swapchain.swapchain_vk_image_views.resize(swapchain_image_count);
	for (uint32_t swapchain_image_idx = 0; swapchain_image_idx < swapchain_image_count; ++swapchain_image_idx) {
		auto swapchain_image = internal->swapchain.swapchain_vk_images[swapchain_image_idx].image;
		...
		VkImageViewCreateInfo image_view_create_info {
			...
			.subresourceRange = {
				...
				.levelCount = 1,
				.baseArrayLayer = 0,
				.layerCount = uint32_t(internal->views.size()),
			},
		};
		VK_CALL_RET(vkCreateImageView(vk_dev->device, &image_view_create_info, nullptr,
									  &internal->swapchain.swapchain_vk_image_views[swapchain_image_idx]),
					"swapchain image view creation failed", false)
		set_vulkan_debug_label(*vk_dev, VK_OBJECT_TYPE_IMAGE_VIEW,
							   uint64_t(internal->swapchain.swapchain_vk_image_views[swapchain_image_idx]),
							   "vr_swapchain_image_#" + std::to_string(swapchain_image_idx));
		
		external_vulkan_image_info info {
			.image = swapchain_image,
			.image_view = internal->swapchain.swapchain_vk_image_views[swapchain_image_idx],
			.format = swapchain_format,
			.access_mask = VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT,
			.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
			.image_base_type = (IMAGE_TYPE::IMAGE_2D_ARRAY |
...
vr_context::swapchain_info_t openxr_context::get_swapchain_info() const {
	if (internal->swapchain.swapchain_images.empty()) {
		throw std::runtime_error("swapchain has not been initialized");
	}
	return {
		.image_count = uint32_t(internal->swapchain.swapchain_images.size()),
		.image_type = internal->swapchain.swapchain_images[0]->get_image_type(),
	};
}

std::string openxr_context::get_vulkan_instance_extensions() const {
	return {}; // not needed for XR_KHR_vulkan_enable2
}

std::string openxr_context::get_vulkan_device_extensions([[maybe_unused]] VkPhysicalDevice_T* physical_device) const {
	return {}; // not needed for XR_KHR_vulkan_enable2
}

std::vector<std::shared_ptr<event_object>> openxr_context::handle_input() {
	std::vector<std::shared_ptr<event_object>> events;
...
device_image* openxr_context::acquire_next_image() REQUIRES(!view_states_lock) {
	internal->cur_swapchain_image = nullptr; // reset
	...
	internal->cur_frame_state = XrFrameState {
		.type = XR_TYPE_FRAME_STATE,
		.next = nullptr,
	};
	XR_CALL_RET(xrWaitFrame(session, &frame_wait_info, &internal->cur_frame_state),
	...
	internal->cur_layer_projection_views = {{
		{
	...
	internal->cur_layer_projection = {
		.type = XR_TYPE_COMPOSITION_LAYER_PROJECTION,
		.next = nullptr,
	};
	
	if (!internal->cur_frame_state.shouldRender) {
		return nullptr;
	}

	internal->cur_layer_projection.layerFlags = 0;
	internal->cur_layer_projection.space = scene_space;
	internal->cur_layer_projection.viewCount = uint32_t(internal->views.size());
	internal->cur_layer_projection.views = internal->cur_layer_projection_views.data();
	
	const XrViewLocateInfo view_locate_info {
		.type = XR_TYPE_VIEW_LOCATE_INFO,
		.next = nullptr,
		.viewConfigurationType = XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
		.displayTime = internal->cur_frame_state.predictedDisplayTime,
		.space = scene_space,
	};
	...
	XR_CALL_RET(xrLocateSpace(view_space, scene_space, internal->cur_frame_state.predictedDisplayTime, &space_location),
	...
	
	{
		...
		for (uint32_t view_index = 0; view_index < view_count; ++view_index) {
			view_states[view_index].position = {
				internal->views[view_index].pose.position.x,
				internal->views[view_index].pose.position.y,
				internal->views[view_index].pose.position.z
			};
			view_states[view_index].orientation = {
				internal->views[view_index].pose.orientation.x,
				internal->views[view_index].pose.orientation.y,
				internal->views[view_index].pose.orientation.z,
				internal->views[view_index].pose.orientation.w
			};
			// store in the same order as OpenVR
			view_states[view_index].fov = {
				internal->views[view_index].fov.angleLeft,
				internal->views[view_index].fov.angleRight,
				internal->views[view_index].fov.angleUp,
				internal->views[view_index].fov.angleDown
			};
		}
	}
	
	for (uint32_t view_index = 0; view_index < view_count; ++view_index) {
		internal->cur_layer_projection_views[view_index].pose = internal->views[view_index].pose;
		internal->cur_layer_projection_views[view_index].fov = internal->views[view_index].fov;
		internal->cur_layer_projection_views[view_index].subImage = XrSwapchainSubImage {
			.swapchain = internal->swapchain.swapchain,
			.imageRect = {
				.offset = { 0, 0 },
	...
	XR_CALL_RET(xrAcquireSwapchainImage(internal->swapchain.swapchain, &acq_info, &swapchain_image_idx),
				"failed to acquire swapchain image", nullptr);
	if (swapchain_image_idx >= internal->swapchain.swapchain_vk_images.size()) {
	...
	XR_CALL_RET(xrWaitSwapchainImage(internal->swapchain.swapchain, &wait_info),
				"failed to wait for swapchain image", nullptr);

	internal->cur_swapchain_image = internal->swapchain.swapchain_images[swapchain_image_idx].get();
	return internal->cur_swapchain_image;
}

bool openxr_context::present([[maybe_unused]] const device_queue& cqueue, const device_image* image) REQUIRES(!view_states_lock) {
#if defined(FLOOR_DEBUG)
	...
	if (internal->cur_swapchain_image != image) {
		log_error("the image provided to present() is not the same as the one returned by acquire_next_image()");
		return false;
	}
	
	if (internal->cur_frame_state.shouldRender && image) {
		const XrSwapchainImageReleaseInfo release_info{
			.type = XR_TYPE_SWAPCHAIN_IMAGE_RELEASE_INFO,
			.next = nullptr,
		};
		XR_CALL_RET(xrReleaseSwapchainImage(internal->swapchain.swapchain, &release_info),
					"failed to release swapchain image", false);
	}
	
	// end frame
	std::array<XrCompositionLayerBaseHeader*, 1> layers {{ reinterpret_cast<XrCompositionLayerBaseHeader*>(&internal->cur_layer_projection) }};
	const XrFrameEndInfo frame_end_info {
		.type = XR_TYPE_FRAME_END_INFO,
		.next = nullptr,
		.displayTime = internal->cur_frame_state.predictedDisplayTime,
		.environmentBlendMode = XR_ENVIRONMENT_BLEND_MODE_OPAQUE,
		.layerCount = internal->cur_frame_state.shouldRender ? 1u : 0u,
		.layers = internal->cur_frame_state.shouldRender ? layers.data() : nullptr,
	};
...
std::optional<XrPath> openxr_context::to_path(const std::string& str) {
	XrPath path { 0 };
...
std::optional<XrPath> openxr_context::to_path(const char* str) {
	XrPath path { 0 };
	XR_CALL_RET(xrStringToPath(instance, str, &path),
				"failed to convert string \"" + (str ? std::string(str) : "nullptr") + "\" to XrPath", {});
	return path;
}
...
XrPath openxr_context::to_path_or_throw(const char* str) {
	...
	XR_CALL_ERR_EXEC(xrStringToPath(instance, str, &path),
					 "failed to convert string \"" + (str ? std::string(str) : "nullptr") + "\" to XrPath",
					 throw std::runtime_error("failed to convert string \"" + (str ? std::string(str) : "nullptr") +
											  "\" to XrPath"););
	return path;
}

std::optional<std::string> openxr_context::path_to_string(const XrPath& path) {
