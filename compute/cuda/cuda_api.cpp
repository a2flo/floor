...
bool cuda_api_init() {
	...
	lib_handle_type cuda_lib_ptr = open_dynamic_library(cuda_lib_name);
	if(cuda_lib_ptr == nullptr) {
	...
	(void*&)cuda_api.graphics_gl_register_image = load_symbol(cuda_lib_ptr, "cuGraphicsGLRegisterImage");
	...
	(void*&)cuda_api.graphics_map_resources = load_symbol(cuda_lib_ptr, "cuGraphicsMapResources");
	if(cuda_api.graphics_map_resources == nullptr) log_error("failed to retrieve function pointer for \"cuGraphicsMapResources\"");
	
	(void*&)cuda_api.graphics_resource_get_mapped_pointer = load_symbol(cuda_lib_ptr, "cuGraphicsResourceGetMappedPointer_v2");
	...
	(void*&)cuda_api.graphics_sub_resource_get_mapped_array = load_symbol(cuda_lib_ptr, "cuGraphicsSubResourceGetMappedArray");
