...
bool cuda_buffer::release_vulkan_buffer(const compute_queue& cqueue, const vulkan_queue&) {
	if (vk_object_state) {
#if defined(FLOOR_DEBUG)
...
#else
bool cuda_buffer::acquire_vulkan_buffer(const compute_queue&, const vulkan_queue&) {
	return false;
}
bool cuda_buffer::release_vulkan_buffer(const compute_queue&, const vulkan_queue&) {
	return false;
}
#endif
