...
// TODO: remaining get_*_id() functions
static uint64_t get_global_id(uint32_t dim floor_unused) __attribute__((always_inline, flatten, pure, enable_if(dim == 0, "const 0"))) {
	return __builtin_ptx_read_ctaid_x() * __builtin_ptx_read_ntid_x() + __builtin_ptx_read_tid_x();
}
static uint64_t get_global_id(uint32_t dim floor_unused) __attribute__((always_inline, flatten, pure, enable_if(dim == 1, "const 1"))) {
	return __builtin_ptx_read_ctaid_y() * __builtin_ptx_read_ntid_y() + __builtin_ptx_read_tid_y();
}
static uint64_t get_global_id(uint32_t dim floor_unused) __attribute__((always_inline, flatten, pure, enable_if(dim == 2, "const 2"))) {
	return __builtin_ptx_read_ctaid_z() * __builtin_ptx_read_ntid_z() + __builtin_ptx_read_tid_z();
}
static uint64_t get_global_id(uint32_t dim floor_unused) __attribute__((always_inline, flatten, pure, enable_if(dim > 2, "invalid dim"), unavailable("invalid dim"))) {
	return 42;
}
static uint64_t get_global_id(uint32_t dim) __attribute__((always_inline, flatten, pure)) {
	switch(dim) {
