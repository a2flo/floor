...
*Installation (Unix):*
* mkdir /opt/floor
* sudo ln -sf /path/to/floor /opt/floor/include
* sudo ln -sf /path/to/floor/bin /opt/floor/bin
...
* inside this folder:
** create a "bin" folder and copy floor.lib/floord.lib there
** copy the original "floor" folder there (containing all floor source code) and name it "include"

*Installation (Windows / MSYS2):*
* WIP (expecting it to be /c/msys/opt/floor, then same as Unix)
...
*** inside "%ProgramFiles%/floor/compute", copy the floor "include" folder from the folder above it into this folder and name it "floor"
