...
** iOS: 9.0+ (7.0+ w/o Metal; 32-bit and 64-bit)
...
** "SDL2":http://www.libsdl.org 2.0.4+ is recommended, earlier versions might work
...
** (opt) Vulkan: requires "Vulkan headers":https://github.com/KhronosGroup/Vulkan-LoaderAndValidationLayers/tree/master/include/vulkan and "ICD loader / SDK":https://vulkan.lunarg.com
...
** to build with libstdc++ instead of libc++:
   "./build.sh libstdc++"
** to build with experimental C++17 support (requires Clang 3.9+):
   "./build.sh c++17"
...
* WIP, currently unsupported due to gcc/clang incompatibilities (expecting it to be /opt/floor, then same as Unix)
...
* compile either toolchain:
** current 3.8.1 toolchain: cd floor/etc/llvm38/ && ./build.sh
** legacy 3.5.2 toolchain: cd floor/etc/llvm35/ && ./build.sh
