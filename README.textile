...
* install all necessary libraries and programs, including clang 3.4+ and libc++ (currently 1101) (-> Credits)
...
on Windows/MinGW:
* install custom MinGW builds (with clang 3.4+ and libc++)
...
on OS X:
* install Xcode 5.1+, Xquartz, SDL2, SDL2_image, CUDA (optionally) and boost (copying headers to /usr/local/include or /usr/include suffices)
...
* compile floor.xcodeproj (floor target)
...
on iOS:
* install Xcode 5.1+
* compile static libs for SDL2, SDL2_image and OpenSSL, then symlink or copy them into your iPhoneOS and iPhoneSimulator /usr/{lib,include} folders
** NOTE: SDL2 currently doesn't support OpenGL ES 3.0 directly (apply "this patch":https://gist.github.com/a2flo/8175227 to SDL/src/video/uikit/SDL_uikitopenglview.m)
...
Right now, only CUDA and OpenCL can be disabled:
* to disable CUDA: don't define "FLOOR_CUDA_CL"
