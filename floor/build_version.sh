...
NEW_SOURCE_COUNT=$(find . -type f \( -name '*.h' -o -name '*.hpp' -o -name '*.cpp' -o -name '*.mm' -o -name '*.c' -o -name '*.m' \) -newer floor/build_version | grep -v "build_version.h" | wc -l | tr -cd [:digit:])
