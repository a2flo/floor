...
unique_ptr<char[]> vr_context::get_vulkan_instance_extensions() const {
	if (compositor == nullptr) {
		return {};
	}
	...
	auto vk_instance_exts = make_unique<char[]>(vk_instance_exts_len);
	compositor->GetVulkanInstanceExtensionsRequired(vk_instance_exts.get(), vk_instance_exts_len);
	return vk_instance_exts;
}

unique_ptr<char[]> vr_context::get_vulkan_device_extensions(VkPhysicalDevice_T* physical_device) const {
	if (compositor == nullptr) {
		return {};
	}
	...
	auto vk_dev_exts = make_unique<char[]>(vk_dev_exts_len);
	compositor->GetVulkanDeviceExtensionsRequired(physical_device, vk_dev_exts.get(), vk_dev_exts_len);
	return vk_dev_exts;
}

bool vr_context::update() {
...
vector<shared_ptr<event_object>> vr_context::update_input() {
...
bool vr_context::present(const compute_queue& cqueue, const compute_image& image) {
...
float4 vr_context::get_projection_raw(const VR_EYE& eye) const {
	float4 ret;
	system->GetProjectionRaw(eye_to_eye(eye), &ret.x, &ret.y, &ret.z, &ret.w);
	return ret;
}

matrix4f vr_context::get_eye_matrix(const VR_EYE& eye) const {
...
const matrix4f& vr_context::get_hmd_matrix() const {
	return hmd_mat;
}
